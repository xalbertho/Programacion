...
class ZumbidoThread(QThread):
    ...
    def run(self):
        try:
            ...
            mensaje_receptor = f"<zumbido>:Te ha enviado un zumbido"
...
class MainWindow(QMainWindow, Ui_MainWindow,Ui_Widget1):
    ...
    def enviar_zumbido(self):
        recipient = "recipient_username"  # Reemplaza con el nombre de usuario del receptor
        self.zumbido_thread = ZumbidoThread(server, self.current_chat_user, recipient)
    ...
    def mensage_entrante(self, mensaje):
        ...
        if mensaje.startswith("<zumbido>"):
            ...
            if remitente != self.current_chat_user:
                self.agregar_mensaje_a_lista_mensajes(contenido, remitente)
                self.vibrate_signal.emit()  # Emitir la señal de vibración
            else:
                pass
