...
class ZumbidoThread(QThread):
    global connected
    signal_zumbido = pyqtSignal(str)

    def __init__(self, server, username, recipient, parent=None):
        global connected
    ...
    def run(self):
        global connected
        try:
            # Enviar mensaje de zumbido al receptor
            mensaje_receptor = "Te ha enviado un zumbido"
            self.server.send(bytes(mensaje_receptor, 'utf-8'))

            # Enviar mensaje de confirmación al remitente
            mensaje_remitente = "Has enviado un zumbido"
            self.signal_zumbido.emit(mensaje_remitente)
        except Exception as e:
            print(f"Error al enviar zumbido: {str(e)}")


class LEDBlinkingThread(QThread):
    signal_stop_blink = pyqtSignal()  # Define la señal como un atributo de clase

    def __init__(self, board, pin, parent=None):
        super().__init__(parent)
        self.board = board
        self.pin = pin
        self._running = True
    ...
    def run(self):
        blink_count = 0
        while self._running and blink_count < 3:
            self.board.digital[self.pin].write(1)
            self.msleep(500)  # Utiliza msleep en lugar de time.sleep para threads de PyQt
            self.board.digital[self.pin].write(0)
            self.msleep(500)
            blink_count += 1
        self.signal_stop_blink.emit()  # Emitir señal cuando termina de parpadear
...
class MainWindow(QMainWindow, Ui_MainWindow,Ui_Widget1):
    vibrate_signal = pyqtSignal()
    def __init__(self, *parent, **flags) -> None:
        super().__init__(*parent, **flags)
        self.setupUi(self)
        self.current_chat_user = None 
        self.recipient = None 
        self.zumbido_thread = None
        self.zumbido_thread = ZumbidoThread(server, self.current_chat_user, self.recipient)
        self.zumbido_thread.signal_zumbido.connect(self.shake_window)
        self.vibrate_signal.connect(self.shake_window)
        self.zumbido_3.clicked.connect(self.enviar_zumbido)

        
        #self.board = pyfirmata.Arduino('COM10')
        #self.led_thread = LEDBlinkingThread(self.board, 13)  # Reemplaza 13 con el número de pin correcto
        #self.led_thread.signal_stop_blink.connect(self.led_blink_finished)
  
        self.cerrarBtn1.clicked.connect(lambda: self.toggle_menu(self.mainmenu, self.cerrarBtn1, "Cerrar"))
        self.cerrarBtn2.clicked.connect(self.cerrar_menu_right)
    ...
    def shake_window(self):
        ...
        print(f"Posición original: {original_pos}")  # Agrega este print para verificar la posición original

        for _ in range(shakes):
            for offset in offsets:
                new_pos = original_pos + offset
                print(f"Nueva posición: {new_pos}")  # Agrega este print para verificar la nueva posición
                self.move(new_pos)
                QtCore.QThread.msleep(int(duration / (shakes * len(offsets))))

        self.move(original_pos)
        print(f"Posición final: {original_pos}")  # Agrega este print para verificar la posición final

    def cerrar_menu_right(self):
        self.rightmenu.hide()
    ...
    def enviar_zumbido(self):
        if self.zumbido_thread is None or not self.zumbido_thread.isRunning():
            self.zumbido_thread = ZumbidoThread(server, self.current_chat_user, self.recipient)
            self.zumbido_thread.signal_zumbido.connect(self.shake_window)
            self.zumbido_thread.start()

    def mostrar_confirmacion_zumbido(self, mensaje):
        print(mensaje)
    ...
    def led_blink_finished(self):
        pass

    def mensage_entrante(self, mensaje):
        print(f"Mensaje recibido: {mensaje}")  # Agrega este print para verificar el mensaje recibido
        if mensaje.startswith("<zumbido>"):
            print("Mensaje de zumbido recibido")  # Agrega este print para verificar que entra en esta condición
            _, remitente, contenido = mensaje.split(':', 2)
            remitente = remitente.strip().removeprefix("De ")
            if remitente != self.current_chat_user:  # Verificar si el remitente no es el usuario actual
                self.agregar_mensaje_a_lista_mensajes(contenido, remitente)
                self.vibrate_signal.emit()
                self.shake_window()  # Emitir la señal de vibración solo si el mensaje es para el usuario actual
            else:
                pass
        if mensaje.startswith("<privado>"):
           # self.led_thread.start()
            ...
            if remitente in self.contact_to_page:
                ...
                # Guardar el mensaje en pending_messages si el remitente no está en contactos
                if remitente not in self.pending_messages:
                    self.pending_messages[remitente] = []
                self.pending_messages[remitente].append(contenido)
                print(f"Remitente no reconocido o chat no iniciado. Mensaje guardado en pending_messages: {contenido}")
