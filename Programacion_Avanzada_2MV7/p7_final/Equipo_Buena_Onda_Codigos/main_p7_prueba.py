...
class ZumbidoThread(QThread):
    ...
    def run(self):
        try:
            ...
            mensaje_receptor = f"<zumbido>:De {self.username}:Te ha enviado un zumbido"
...
class MainWindow(QMainWindow, Ui_MainWindow, Ui_Widget1):
    ...
    def enviar_zumbido(self):
        if self.zumbido_thread is None or not self.zumbido_thread.isRunning():
            self.zumbido_thread = ZumbidoThread(server, self.username, self.recipient)
    ...
    def conectar(self, dialog):
        ...
        self.username = usuario  # Agregar esta línea
    ...
    def mensage_entrante(self, mensaje):
        ...
        if mensaje.startswith("<zumbido>"):
            ...
            if remitente != self.username:
                self.agregar_mensaje_a_lista_mensajes(contenido, remitente)
                self.vibrate_signal.emit()
                self.shake_window()
            else:
                self.mostrar_confirmacion_zumbido(f"Has enviado un zumbido a {self.recipient}")
