    ...
    while True:
        try:
            ...
            if message:
                ...
                elif msg.startswith('<zumbido>'):
                    # El formato esperado es '<zumbido>:recipient_name'
                    _, recipient_name = msg.split(':', 1)
                    sender_name = getName(conn)
                    send_vibration(recipient_name, sender_name, conn)
                else:
                    name = getName(conn)
...
def send_vibration(sender_name, recipient_name, conn):
    ...
    if recipient and recipient.name.lower() != sender_name.lower():
        try:
            vibration_message = f"<zumbido>:De {sender_name}:Te ha enviado un zumbido"
            recipient.conn.send(bytes(vibration_message, 'utf-8'))
        except Exception as e:
            print(f"Error sending vibration message to {recipient_name}: {e}")
            conn.send(bytes(f"Error sending vibration to {recipient_name}.", 'utf-8'))
    else:
        conn.send(bytes(f"No se puede enviar vibración a sí mismo.", 'utf-8'))
